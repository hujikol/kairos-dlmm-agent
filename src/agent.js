...
const MANAGER_TOOLS  = new Set(["close_position", "claim_fees", "swap_token", "update_config", "get_position_pnl", "get_my_positions", "set_position_note", "add_pool_note", "get_wallet_balance", "get_wallet_positions"]);
const SCREENER_TOOLS = new Set(["deploy_position", "get_active_bin", "get_top_candidates", "check_smart_wallets_on_pool", "get_token_holders", "get_token_narrative", "get_token_info", "search_pools", "get_pool_memory", "add_pool_note", "add_to_blacklist", "update_config", "get_wallet_balance", "get_my_positions", "get_wallet_positions"]);
...
// Single INTENT_PATTERNS definition lives below (after DEFAULT_MODEL) — deduplicated
...
const DEFAULT_MODEL = process.env.LLM_MODEL || "hermes-3-405b";

// Canonical source of truth — INTENTS and TOOL_REQUIRED_INTENTS both derive from this
...
const TOOL_REQUIRED_INTENTS = new RegExp(INTENT_PATTERNS.map(p => p.re.source).join("|"), "i");
...
export async function agentLoop(goal, maxSteps = null, sessionHistory = [], agentType = "GENERAL", model = null, maxOutputTokens = null, options = {}) {
  ...
  for (let step = 0; step < effectiveMaxSteps; step++) {
    ...
    try {
      ...
      const FALLBACK_MODEL = process.env.LLM_FALLBACK_MODEL || "minimax/minimax-01";
      let response;
        ...
        for (const tc of msg.tool_calls) {
          if (tc.function?.arguments) {
            try {
              ...
              log("debug", "agent", `First JSON.parse failed for ${tc.function?.name}: ${e?.message}`);
              try {
      ...
      // Tool call overflow guard
      toolCallsThisStep += msg.tool_calls.length;
      ...
      const toolResults = await Promise.all(msg.tool_calls.map(async (toolCall) => {
        ...
        try {
          ...
          log("debug", "agent", `JSON.parse failed: ${parseErr?.message}`);
          try {
