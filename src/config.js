...
export const config = {
  ...
  gmgn: {
    ...
    requestDelayMs: gmgnUserConfig?.requestDelayMs ?? u.gmgnRequestDelayMs ?? (process.env.GMGN_REQUEST_DELAY_MS ? Number(process.env.GMGN_REQUEST_DELAY_MS) : 2500),
    maxRetries:    gmgnUserConfig?.maxRetries    ?? u.gmgnMaxRetries    ?? (process.env.GMGN_MAX_RETRIES    ? Number(process.env.GMGN_MAX_RETRIES)    : 2),
