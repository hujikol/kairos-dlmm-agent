/**
 * Study top LPers for a pool via Agent Meridian relay.
 * Routes through Agent Meridian — no LPAgent API key needed on user side.
 *
 * Data flow: kairos → Agent Meridian → LPAgent API
...
export async function studyTopLPers({ pool_address, limit = 4 }) {
  try {
    const [poolData, signalData] = await Promise.all([
      agentMeridianJson(`/top-lp/${pool_address}`),
      agentMeridianJson(`/study-top-lp/${pool_address}`),
    ]);
    ...
    return {
      pool: pool_address,
      pool_name: poolData.overview?.name || `${poolData.overview?.tokenXSymbol || "TOKEN"}-${poolData.overview?.tokenYSymbol || "SOL"}`,
      message: "LPAgent-backed top LP study from Agent Meridian (30 min cached).",
      patterns,
      lpers,
    };
  } catch (err) {
    return {
      pool: pool_address,
      message: `Agent Meridian relay error: ${err?.message ?? err}`,
      patterns: {},
      lpers: [],
    };
  }
...
function buildPatterns(ranked, historicalOwners, signalData, overview) {
  ...
  return {
    ...
    pool_name: overview.name || `${overview.tokenXSymbol || "TOKEN"}-${overview.tokenYSymbol || "SOL"}`,
